 ...
 * nat 20/nat 1 do d20 mantido; TODO passo de dano `critable` do mesmo
 * raio rola com crit (dados dobrados) no nat 20 e é PULADO no nat 1
 * (pacote não-critable — ex.: explosão da Faca de Gelo — rola sempre).
 * `mode` ('adv'|'dis' — Shift/Alt no clique) vale só pros ataques.
 *
...
export function executeCastPlan(steps, roll, { mode } = {}) {
  ...
  for (const step of steps) {
    if (step.kind === 'attack') {
      ...
      if (step.critable && pendingAttack?.nat1) continue
